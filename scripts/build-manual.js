...
var BLOCKS = [
  ...
  p("PRINCIPLE: the PROMISE of disease in the natal charts (D1 Rasi + KP + D6 Shashthamsa) is primary and carries the most weight - it also determines the KIND of disease (the body parts and ailments highlighted). Dasha and transits are TRIGGERS that set the timing and carry far less weight on their own. When a trigger activates a planet/area that is already promised, that is a CONFLUENCE and the severity is escalated."),
  h3("Promise (primary - natal charts)"),
  li("KP significator of disease houses: 6 -> +2.0, 8 -> +1.3, 12 -> +1.3 to the promise of that planet."),
  li("D1 dusthana placement +1.2; uncancelled debilitation +1.2; 6th-lord +1.2, 8th-lord +1.0."),
  li("D6 6th (disease) +1.3; D6 8th +0.9. Weak in Shadbala +0.6. 22nd-Drekkana / 64th-Navamsa lord +1.0 each."),
  li("Body parts and ailments are distributed from the PROMISED planets (primary karaka x1.4, occupied-sign Kalapurusha x0.5), so the predicted 'kind of disease' is chart-led."),
  h3("Triggers (secondary - timing)"),
  li("Maha Dasha lord weight 1.0, Antar 0.8, Pratyantar 0.5."),
  li("Transit of Saturn/Mars/Rahu/Ketu over natal houses 1/6/8/12: +0.5 each; Saturn over the natal Moon (Sade Sati): +0.5."),
  li("A trigger planet that carries NO promised disease has only a small effect on the 'kind of disease' (weight x0.4)."),
